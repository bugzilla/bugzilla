...
sub process_interdiff {
    ...
    if ( $ENV{MOD_PERL} ) {
        ...
        $pid = open3( gensym, $interdiff_stdout, $interdiff_stderr, $lc->{interdiffbin}, $old_filename, $new_filename );
        $use_select = 1;
    }
    ...
    $reader->iterate_string( 'interdiff #' . $old_attachment->id . ' #' . $new_attachment->id, $stdout );
}
...
sub setup_template_patch_reader {
    ...
    $last_reader->sends_data_to(
        new PatchReader::DiffPrinter::template(
