...
sub bz_setup_database {
    ...
    if ( $attach_creation && $attach_creation->{TYPE} =~ /^TIMESTAMP/i ) {
        ...
        $sth = $self->prepare( "SELECT bug_id, attach_id, submitter_id " . "FROM attachments" );
        $sth->execute();
        ...
        while ( my ( $bug_id, $attach_id, $submitter_id ) = $sth->fetchrow_array() ) {
            ...
            if ($when) {
                $self->do( "UPDATE attachments " . "SET creation_ts='$when' " . "WHERE attach_id=$attach_id" );
            }
            else {
        ...
        $self->bz_alter_column( "attachments", "creation_ts", { TYPE => 'DATETIME', NOTNULL => 1 } );
    }
    ...
    if ( $login_lastused && $login_lastused->{TYPE} =~ /^TIMESTAMP/i ) {
        $self->bz_alter_column( 'logincookies', 'lastused', { TYPE => 'DATETIME', NOTNULL => 1 } );
    }
