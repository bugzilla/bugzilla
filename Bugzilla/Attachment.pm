...
sub _check_data {
    ...
    my $max_size
        = max( Bugzilla->params->{'maxlocalattachment'} * 1048576, Bugzilla->params->{'maxattachmentsize'} * 1024 );
...
sub _check_filename {
    ...
    $filename = substr( $filename, -&MAX_ATTACH_FILENAME_LENGTH, MAX_ATTACH_FILENAME_LENGTH );
...
sub validate_can_edit {
    ...
    return (
        $attachment->attacher->id == $user->id || ( ( !$attachment->isprivate || $user->is_insider )
