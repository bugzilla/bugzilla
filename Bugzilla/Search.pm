...
sub _standard_where {
    ...
    if ( Bugzilla->params->{'or_groups'} ) {
        $security_term
            .= " (security_map.group_id IS NULL OR security_map.group_id IN (" . $user->groups_as_string . "))";
    }
    else {
...
sub _get_column_joins {
    ...
    Bugzilla::Hook::process( 'buglist_column_joins', { column_joins => \%column_joins } );
...
sub _alias_nonchanged {
    ...
    $self->_do_operator_function($args);
    $args->{term} = build_subselect( "bugs.bug_id", "bugs_aliases.bug_id", "bugs_aliases", $args->{term} );
}
...
sub _multiselect_table {
    ...
    if ( $field eq 'keywords' ) {
        ...
        $args->{_extra_where} = " AND attachments.isprivate = 0"
            if !$self->_user->is_insider;
        return "attachments INNER JOIN attach_data " . " ON attachments.attach_id = attach_data.id";
    }
