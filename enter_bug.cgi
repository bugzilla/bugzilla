...
if ( $product_name eq '' ) {
    ...
    my $classification = Bugzilla->params->{'useclassification'} ? scalar( $cgi->param('classification') ) : '__all';
...
if ($cloned_bug_id) {
    ...
    $default{'rep_platform'} = formvalue( 'rep_platform', Bugzilla->params->{'defaultplatform'} || detect_platform() );
    $default{'op_sys'}       = formvalue( 'op_sys', Bugzilla->params->{'defaultopsys'} || detect_op_sys() );
