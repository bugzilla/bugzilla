...
sub update_fielddefs_definition {
    ...
    $dbh->bz_add_column( 'fielddefs', 'long_desc', { TYPE => 'varchar(255)', NOTNULL => 1, DEFAULT => "''" }, '' );
...
sub update_table_definitions {
    ...
    $dbh->bz_alter_column( 'logincookies', 'cookie', { TYPE => 'varchar(16)', PRIMARYKEY => 1, NOTNULL => 1 } );
    ...
    $dbh->bz_alter_column( 'attachments', 'filename', { TYPE => 'varchar(255)', NOTNULL => 1 } );
    ...
    $dbh->bz_alter_column( 'bugs', 'target_milestone', { TYPE => 'varchar(64)', NOTNULL => 1, DEFAULT => "'---'" } );
...
sub _use_ids_for_products_and_components {
    ...
    if ( $dbh->bz_column_info( "products", "product" ) ) {
        ...
        $dbh->bz_add_column( "components", "id", { TYPE => 'MEDIUMSERIAL', NOTNULL => 1, PRIMARYKEY => 1 } );
...
sub _add_password_salt_separator {
    ...
    my $profiles = $dbh->selectall_arrayref(
        "SELECT userid, cryptpassword FROM profiles WHERE (" . $dbh->sql_regexp( "cryptpassword", "'^[^,]+{'" ) . ")" );
...
sub _fix_components_primary_key {
    ...
    if ( $dbh->bz_column_info( 'components', 'id' )->{TYPE} ne 'MEDIUMSERIAL' ) {
        ...
        $dbh->bz_alter_column( "components", "id", { TYPE => 'MEDIUMSERIAL', NOTNULL => 1, PRIMARYKEY => 1 } );
        ...
        $dbh->bz_alter_column( "component_cc",   "component_id", { TYPE => 'INT3', NOTNULL => 1 } );
    }
}
