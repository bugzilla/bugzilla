...
sub DATE_COLUMNS {
    my @fields = ( @{ Bugzilla->fields( { type => [ FIELD_TYPE_DATETIME, FIELD_TYPE_DATE ] } ) } );
...
sub _extract_bug_ids {
    ...
    foreach my $comment (@$comments) {
        ...
        push @bug_ids,
            $text =~ /(?<=^\*\*\*\ This\ bug\ has\ been\ marked\ as\ a\ duplicate\ of\ )([0-9]+)(?=\ \*\*\*\Z)/;
    }
...
sub update {
    ...
    foreach my $alias (@$removed_aliases) {
        $dbh->do( 'DELETE FROM bugs_aliases WHERE bug_id = ? AND alias = ?', undef, $self->id, $alias );
    }
    foreach my $alias (@$added_aliases) {
        trick_taint($alias);
        $dbh->do( 'INSERT INTO bugs_aliases (bug_id, alias) VALUES (?,?)', undef, $self->id, $alias );
    }
...
sub _send_bugmail {
    ...
    my $results = Bugzilla::BugMail::Send( $params->{'id'}, $params->{'forced'}, $params );
...
sub _check_commenton {
    ...
    my $is_changing
        = ref($invocant)
        ? $invocant->$field ne $new_value
        : $new_value ne '';

    if ( $is_changing && !$has_comment ) {
        my $old_value = ref($invocant) ? $invocant->$field : undef;
        ThrowUserError( 'comment_required', { field => $field, old => $old_value, new => $new_value } );
    }
}
...
sub _check_dependencies {
    ...
    if ( !ref $invocant ) {
        ...
        return ( [], [] ) unless Bugzilla->user->in_group( 'editbugs', $params->{product}->id );
    }
...
sub _check_resolution {
    ...
    my $status
        = ref($invocant)
        ? $invocant->status->name
        : $params->{bug_status};
    my $is_open
        = ref($invocant)
        ? $invocant->status->is_open
        ...
        )
    {
        ...
        if ($count_open) {
            ...
            ThrowUserError( "still_unresolved_bugs", { bug_id => $bug_id, dep_count => $count_open } );
        }
    }
...
sub set_all {
    ...
    if ( exists $params->{alias} && $params->{alias}{set} ) {
        my ( $removed_aliases, $added_aliases ) = diff_arrays( $self->alias, $params->{alias}{set} );
        $params->{alias} = {
            add    => $added_aliases,
...
sub alias {
    ...
    my $dbh = Bugzilla->dbh;
    $self->{'alias'} = $dbh->selectcol_arrayref( q{SELECT alias FROM bugs_aliases WHERE bug_id = ? ORDER BY alias},
        undef, $self->bug_id );
...
sub default_bug_status {
    ...
    if ( scalar(@statuses) == 1 ) {
        ...
        $status = ( $statuses[0]->name ne 'UNCONFIRMED' ) ? $statuses[0]->name : $statuses[1]->name;
    }
...
sub groups {
    ...
    while ( my ( $groupid, $name, $description, $ison, $ingroup, $isactive, $membercontrol, $othercontrol )
        = $sth->fetchrow_array() )
    {
...
sub bug_alias_to_id {
    ...
    my $dbh = Bugzilla->dbh;
    trick_taint($alias);
    return $dbh->selectrow_array( "SELECT bug_id FROM bugs_aliases WHERE alias = ?", undef, $alias );
}
...
sub _bugs_in_order {
    ...
    my $rows = $dbh->selectall_arrayref(
        'SELECT bug_id, alias FROM bugs_aliases WHERE ' . $dbh->sql_in( 'bug_id', $bug_ids ) . ' ORDER BY alias' );
...
sub LogActivityEntry {
    my ( $bug_id, $field, $removed, $added, $user_id, $timestamp, $comment_id, $attach_id ) = @_;
    ...
    while ( $removed || $added ) {
        ...
        $sth->execute( $bug_id, $user_id, $timestamp, $fieldid, $removestr, $addstr, $comment_id, $attach_id );
    }
}
