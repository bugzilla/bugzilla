...
use constant FEATURE_FILES => (
    ...
    rest      => [ 'Bugzilla/WebService/Server/REST.pm', 'rest.cgi', 'Bugzilla/WebService/Server/REST/Resources/*.pm' ],
    moving    => ['importxml.pl'],
...
sub _missing_apache_modules {
    ...
    my $retval = $?;
    if ( $retval > 0 ) {
        print STDERR install_string( 'apachectl_failed', { command => $command, root => ROOT_USER } ), "\n";
        return [];
    }
    my @missing;
