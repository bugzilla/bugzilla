...
my @translation
    = ( [ NEW => 'CONFIRMED' ], [ ASSIGNED => 'IN_PROGRESS' ], [ REOPENED => 'CONFIRMED' ], [ CLOSED => 'VERIFIED' ], );
...
foreach my $pair (@translation) {
    ...
    $dbh->do( 'DELETE FROM bugs_activity WHERE fieldid = ? AND added = removed', undef, $status_field->id );
    ...
    Bugzilla::Series->Bugzilla::Search::Saved::rename_field_value( 'bug_status', $from, $to );
}
if ($enable_unconfirmed) {
