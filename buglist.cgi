...
sub _get_common_flag_types {
    ...
    my @show_flag_types        = grep { $user->can_request_flag($_) } values %common_flag_types;
...
ORDER: for ($order) {
        ...
        /^Last Changed$/ && do {
            @order_columns = ( "changeddate", "bug_status", "priority", "assigned_to", "bug_id" );
            last ORDER;
        };
        do {
...
if ( $dotweak && scalar @bugs ) {
    ...
    if ($one_product) {
        ...
        if ( Bugzilla->params->{'usetargetmilestone'} ) {
            $vars->{'milestones'} = [ map( $_->name, grep( $_->is_active, @{ $one_product->milestones } ) ) ];
        }
    }
    else {
