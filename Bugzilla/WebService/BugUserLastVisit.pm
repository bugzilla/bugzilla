...
sub update {
    ...
    foreach my $bug_id (@$ids) {
        ...
        push( @results, $self->_bug_user_last_visit_to_hash( $bug->id, $last_visit_ts, $params ) );
    }
...
sub get {
    ...
    if ($ids) {
        ...
        @last_visits = map { $self->_bug_user_last_visit_to_hash( $_->bug_id, $_->last_visit_ts, $params ) }
            @{ $user->last_visited };
    }
